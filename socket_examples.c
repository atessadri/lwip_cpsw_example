...
#define TFTP_STACK_SIZE_BYTES     32768
#define TFTP_LST_STACK_SIZE_WORDS (TFTP_STACK_SIZE_BYTES/sizeof(configSTACK_DEPTH_TYPE))
#define TFTP_LST_TASK_PRIORITY    (configMAX_PRIORITIES - 8)
StackType_t gTftpLstTaskStack[TFTP_STACK_SIZE_BYTES] __attribute__((aligned(32)));
StaticTask_t gTftpLstTaskObj;
...
char response[TFTP_MAX_MSG_LEN];

#define TFTP_BUF_LEN       8
char buf[TFTP_BUF_LEN] = "ACK";
...
typedef enum {
    TFTP_RRQ = 1,      ///< TFTP RRQ
    TFTP_WRQ = 2,      ///< TFTP WRQ
    TFTP_DATA = 3,     ///< TFTP Data
    TFTP_ACK = 4,      ///< TFTP ACK
    TFTP_ERROR = 5     ///< TFTP Error
} tftp_opcode;
...
void tftp_set_block(char* packet, volatile u16_t block)
{
...
/** This is an example function that tests
    UDP listen and reply from a different port. */
static void
sockex_testlisten_udp(void *arg)
{
  int s, r;
  int block;
  int ret;
  ...
  addr.sin_port = PP_HTONS(TFTP_PORT);
  //inet_addr_from_ip4addr(&addr.sin_addr, ip_2_ip4(ipaddr));
#endif /* LWIP_IPV6 */
  ...
  r = lwip_socket(AF_INET, SOCK_DGRAM, 0);
#endif /* LWIP_IPV6 */
  LWIP_ASSERT("s >= 0", s >= 0);
  ...
  do {
      n = lwip_recvfrom(s, request, TFTP_MAX_MSG_LEN, 0,
  ...
  do {
      ...
      tftp_set_opcode(response, TFTP_ACK);
      tftp_set_block(response, block);


      n = lwip_sendto(r, response, TFTP_MAX_ACK_LEN, 0, (struct sockaddr *)&from, sizeof(from));
      if ( n < 0 ) {
          printf("Error %d sending ACK for block %d\r\n", n, block);
      }
      ...
      do {
          fromlen = sizeof(struct sockaddr_in);
          n = lwip_recvfrom(r, request, TFTP_MAX_MSG_LEN, 0,
                            (struct sockaddr *) &from, (socklen_t *)&fromlen);
          if ( n < 0 ) {
              printf("Connection timeout waiting block %d\r\n", block);
          }
          printf("Received block %d size %d\r\n", block, n);
      } while ( n < 0 );

      printf("Completed block %d\r\n", block);
  } while (1);
  ...
  printf("sockex_testlisten_udp finished successfully\n");
}
...
void socket_examples_init(void)
{
  ...
  xTaskCreateStatic( socket_example_test,     /* Pointer to the function that implements the task. */
                            "socket_example_test",             /* Text name for the task.  This is to facilitate debugging only. */
                            TFTP_LST_STACK_SIZE_WORDS, /* Stack depth in units of StackType_t typically uint32_t on 32b CPUs */
                            NULL,                      /* We are not using the task parameter. */
                            TFTP_LST_TASK_PRIORITY,    /* task priority, 0 is lowest priority, configMAX_PRIORITIES-1 is highest */
                            gTftpLstTaskStack,         /* pointer to stack base */
                            &gTftpLstTaskObj);          /* pointer to statically allocated task object memory */
#endif
}
